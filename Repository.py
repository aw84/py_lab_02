...
class Repository2(object):
    ...
    def _get_entries(self, stmt):
        ...
        row = stmt.next()
        while row is not None:
            e = self.data_map.create_from_db(row)
            entries.append(e)
            row = stmt.next()
        return entries

    def query_preparation(self, criteria=None):
        if criteria is None:
    ...
    def find_by_criteria(self, criteria):
        query = self.query_preparation(criteria)
        db = Database.database_factory()
        stmt = db.statement(query)
        stmt.execute(criteria.values())
        return self._get_entries(stmt)
